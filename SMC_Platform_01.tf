...
module "Platform_01_Network_Security_Group_Rules" {

  source                      = "git::https://git.health.nsw.gov.au/ehnsw-swdcr/terraform-azurerm-nsg-rules-csv.git"
  network_security_group_name = var.platform_01_network_security_group_name
  resource_group_name         = data.azurerm_resource_group.smc_platform_01.name
  location                    = data.azurerm_resource_group.smc_platform_01.location
  csv_file_name_nsg           = "nsg_rules/${var.platform_01_csv_file_name_nsg}"
...
#**************************************************************
module "Platform_Recovery_Service_Vault" {
  ...
  backup_policy_file_share_name = var.platform_backup_policy_file_share_name != "" ? var.platform_backup_policy_file_share_name : null
  ...
  # VM Policy Details
  backup_policy_vm_name = var.platform_backup_policy_vm_name
  backup_policy_vm = {
    frequency = "Daily"
    time      = "01:00"
  }
